...
// [sdk.Middleware] and [sdk.FunctionProvider]: Wrap supplies management clients
// to non-orchestration invocations, while ProvidedFunctions contributes replay
// adapters and activities to the App. Replay runs as the registered function,
// inside the ordinary middleware chain.
//
...
func (d *Durable) Orchestrator(name string, o task.Orchestrator) *Durable {
	...
	d.provided = append(d.provided, sdk.FunctionRegistration{
		Name: name,
		// A closure retains a source location for function metadata; a bound
		// method value reports an autogenerated trampoline instead.
		Func: func(ctx context.Context, encoded string) (string, error) {
			// The worker binds encoded before middleware runs. Read the carrier
			// at execution time to preserve replacements, including empty input,
			// just as the original Durable.Wrap replay path did. Direct calls
			// without a carrier use the supplied argument.
			if mc, ok := sdk.MiddlewareContextFrom(ctx); ok {
				encoded = mc.InputString()
			}
			return d.runner.loadAndRun(ctx, encoded)
		},
...
// Wrap implements [sdk.Middleware]. All invocations continue through the
// middleware chain. Replay is performed by the registered orchestrator adapter,
// so middleware registered after Durable can observe it too. Only
// non-orchestration invocations receive a management client from this middleware.
